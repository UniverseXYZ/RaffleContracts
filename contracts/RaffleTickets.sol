...
contract RaffleTickets is IRaffleTickets, ERC721 {
  ...
  function getColorString(string memory raffleColorOne, string memory raffleColorTwo) internal pure returns (string memory) {
    string memory colorOne = keccak256(abi.encodePacked(raffleColorOne)) == keccak256(abi.encodePacked('')) ? 'BF953F' : raffleColorOne;
    string memory colorTwo = keccak256(abi.encodePacked(raffleColorTwo)) == keccak256(abi.encodePacked('')) ? 'FCF6BA' : raffleColorTwo;
    string memory paletteOne = string(
      abi.encodePacked('#',colorOne,';#',colorTwo,';#',colorOne)
    );
    string memory paletteTwo = string(
      abi.encodePacked('#',colorTwo,';#',colorOne,';#',colorTwo)
    );

    return string(
      abi.encodePacked(
        '<linearGradient id="golden-grad"><stop offset="0%" stop-color="#',
        colorOne,
        '"><animate attributeName="stop-color" values="',
        paletteOne,'" dur="5s" repeatCount="indefinite" /></stop><stop offset="20%" stop-color="#',
        colorTwo,'"><animate attributeName="stop-color" values="',
        paletteTwo,'" dur="6s" repeatCount="indefinite" /></stop><stop offset="60%" stop-color="#',
        colorOne,'"><animate attributeName="stop-color" values="',
        paletteOne,'" dur="4s" repeatCount="indefinite" /></stop><stop offset="80%" stop-color="#',
        colorTwo,'"><animate attributeName="stop-color" values="',
        paletteTwo,'" dur="6s" repeatCount="indefinite" /></stop><stop offset="100%" stop-color="#',
        colorOne,'"><animate attributeName="stop-color" values="',
        paletteOne,'" dur="5s" repeatCount="indefinite" /></stop></linearGradient>'
      )
    );
  }
